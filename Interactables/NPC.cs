...
public class NPC : MonoBehaviour {
	...
	void LateUpdate(){
		if (inCutscene) {
			if(!camF.cutsceneMode && !camF.fpvMode){
			...
			if(camF.fpvMode){
				camF.transform.forward = Vector3.Lerp(camF.transform.forward, dialogue[currentPage].cameraFocus.position - camF.transform.position, 
				                                      Time.deltaTime * 10f);
			}
		}
	}
