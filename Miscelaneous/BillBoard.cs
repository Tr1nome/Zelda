...
public class BillBoard : MonoBehaviour { 
	...
	void Update() {
		if (!noCam){
			...
			transform.RotateAround(this.transform.position, this.transform.up, 180f);
		}else {
            #if UNITY_EDITOR
            Debug.LogError ("BillBoard.cs: There doesn't seem to be a Main Camera in the scene. Please use the 'Main Camera' prefab from the 'prefabs' folder.");
